...
Mech_7::Mech_7() : mech_7{Mechanism::Mech_7}{
    mech7MotorConfig.MotorOutput.WithNeutralMode(ctre::phoenix6::signals::NeutralModeValue::Coast);
    mech7MotorConfig.MotorOutput.WithInverted(ctre::phoenix6::signals::InvertedValue::CounterClockwise_Positive);
    mech7MotorConfig.CurrentLimits.WithSupplyCurrentLimit(static_cast<units::current::ampere_t>(20));
    mech_7.GetConfigurator().Apply(mech7MotorConfig);
}

void Mech_7::setMech_7(double speed){
    mech_7.Set(speed);
    mech_7.Set(speed);
}